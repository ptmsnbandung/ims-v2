...
<div class="space-y-6" x-data="userManagement()">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 text-xs text-slate-400 mb-1.5 font-medium">
                <span>IMS</span>
                <span>&rsaquo;</span>
                <span>Pengaturan</span>
                <span>&rsaquo;</span>
                <span class="text-blue-400">Manajemen User</span>
            </div>
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-xl bg-blue-500/10 border border-blue-500/30 flex items-center justify-center text-blue-400 shadow-lg shadow-blue-500/10">
                    <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" />
                    </svg>
                </div>
                <div>
                    <h1 class="text-xl sm:text-2xl font-bold text-white tracking-tight">Manajemen User &amp; Hak Akses</h1>
                    <p class="text-xs sm:text-sm text-slate-400">Kelola data pengguna, peran akses (role), dan status akun sistem.</p>
                </div>
            </div>
        </div>
        ...
        <button @click="openCreateModal()"
                class="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold shadow-lg shadow-blue-600/25 transition-all active:scale-95">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="p-5 rounded-2xl bg-slate-900/80 border border-slate-800/80 backdrop-blur-xl flex items-center justify-between relative overflow-hidden group hover:border-slate-700 transition">
            <div class="space-y-1">
                ...
                <p class="text-[11px] text-slate-500">Seluruh akun terdaftar</p>
            </div>
            <div class="w-12 h-12 rounded-2xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-blue-400 shadow-inner">
        ...
        <div class="p-5 rounded-2xl bg-slate-900/80 border border-slate-800/80 backdrop-blur-xl flex items-center justify-between relative overflow-hidden group hover:border-slate-700 transition">
            <div class="space-y-1">
                ...
                <p class="text-[11px] text-slate-500">Dapat login ke sistem</p>
            </div>
        ...
        <div class="p-5 rounded-2xl bg-slate-900/80 border border-slate-800/80 backdrop-blur-xl flex items-center justify-between relative overflow-hidden group hover:border-slate-700 transition">
            <div class="space-y-1">
                ...
                <p class="text-[11px] text-slate-500">Akses login terkunci</p>
            </div>
            <div class="w-12 h-12 rounded-2xl bg-rose-500/10 border border-rose-500/20 flex items-center justify-center text-rose-400 shadow-inner">
                <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
        ...
        <div class="p-5 rounded-2xl bg-slate-900/80 border border-slate-800/80 backdrop-blur-xl flex items-center justify-between relative overflow-hidden group hover:border-slate-700 transition">
            <div class="space-y-1">
                ...
                <p class="text-[11px] text-slate-500">Hak akses dikonfigurasi</p>
            </div>
                    ...
                    @forelse($users as $u)
                        @php
                            ...
                            if (str_contains($namaLvl, 'direktur') || str_contains($namaLvl, 'admin')) {
                                $roleBadge = 'bg-rose-500/10 text-rose-400 border-rose-500/30';
                                $roleLabel = 'Admin';
                            } elseif (str_contains($namaLvl, 'finance')) {
                                $roleBadge = 'bg-amber-500/10 text-amber-400 border-amber-500/30';
                                $roleLabel = 'Finance';
                            } elseif (str_contains($namaLvl, 'noc')) {
                                $roleBadge = 'bg-sky-500/10 text-sky-400 border-sky-500/30';
                                $roleLabel = 'NOC';
                            } elseif (str_contains($namaLvl, 'teknik')) {
                                $roleBadge = 'bg-indigo-500/10 text-indigo-400 border-indigo-500/30';
                                $roleLabel = 'Teknik';
                                ...
                                $roleBadge = 'bg-purple-500/10 text-purple-400 border-purple-500/30';
                        ...
                        <tr class="hover:bg-slate-800/40 transition">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="flex items-center gap-2">
                                            <span class="font-bold text-white text-sm tracking-wide">{{ $u->nama_karyawan }}</span>
                                            @if($isCurrentUser)
                                                <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-blue-500/20 text-blue-400 border border-blue-500/40">
                                                    Akun Anda
                                                </span>
                                            @endif
                                        </div>
                                        <p class="text-[11px] text-slate-400 mt-0.5">
                                            {{ $u->nama_jabatan }} &bull; <span class="font-mono text-slate-500">{{ $u->kode_karyawan ?: '-' }}</span>
                                        </p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-5 py-4 whitespace-nowrap">
                                ...
                                @else
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-rose-500/10 text-rose-400 border border-rose-500/30">
                            ...
                            <td class="px-5 py-4 whitespace-nowrap">
                                @if($u->las_login)
                                    <p class="text-slate-300 font-medium">{{ \Carbon\Carbon::parse($u->las_login)->format('d M Y H:i') }} WIB</p>
                                    <p class="text-[10px] text-slate-500 font-mono">IP: {{ $u->last_ip ?: '127.0.0.1' }}</p>
                                @else
                                    <span class="text-slate-500 italic">Belum pernah login</span>
                                @endif
                            </td>
                            ...
                            <td class="px-5 py-4 whitespace-nowrap text-center">
                                <div class="inline-flex items-center justify-center gap-1.5">
                                    ...
                                    <button @click="openEditModal({{ json_encode($u) }})"
                                            ...
                                            class="p-2 rounded-xl bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 border border-blue-500/30 transition">
                                        <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                    ...
                                    @if(!$isCurrentUser)
                                        <form action="{{ route('admin.users.toggle-status', $u->kode_pengguna) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin mengubah status aktif pengguna {{ $u->username }}?');">
                                            ...
                                            <button type="submit"
                                                    ...
                                                    class="p-2 rounded-xl {{ $u->status_aktif == '1' ? 'bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' }} transition">
                                                @if($u->status_aktif == '1')
                                                    <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                        ...
                                        <form action="{{ route('admin.users.delete', $u->kode_pengguna) }}" method="POST" onsubmit="return confirm('PERINGATAN: Apakah Anda yakin ingin menghapus user {{ $u->username }} secara permanen?');">
                                            ...
                                            <button type="submit"
                                                    ...
                                                    class="p-2 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/30 transition">
                                                <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-12 text-center text-slate-500">
                                <svg class="w-12 h-12 mx-auto mb-3 text-slate-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" />
                                </svg>
            ...
            <div x-show="isCreateOpen"
                ...
                <div class="p-6 border-b border-slate-800 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-base font-bold text-white leading-tight">Tambah User Baru</h3>
                ...
                <form action="{{ route('admin.users.store') }}" method="POST">
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-slate-300 mb-1.5">Nama Lengkap / Karyawan <span class="text-rose-400">*</span></label>
                            <input type="text"
                                   name="nama_lengkap"
                                   required
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-semibold text-slate-300 mb-1.5">Username / Login ID <span class="text-rose-400">*</span></label>
                                <input type="text"
                                       name="username"
                                       required
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-semibold text-slate-300 mb-1.5">Jabatan</label>
                                <input type="text"
                                       name="jabatan"
                        ...
                        <div class="p-4 rounded-xl bg-slate-950/60 border border-slate-800 space-y-3">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                    <label class="block text-[11px] font-medium text-slate-400 mb-1">Password Baru <span class="text-rose-400">*</span></label>
                                    <input type="password"
                                           name="password"
                                           required
                                           minlength="6"
    ...
    <!-- ================================================================= -->
    <!-- MODAL 2: EDIT DATA PENGGUNA (Matching Screenshot 2)               -->
    <!-- ================================================================= -->
            ...
            <div x-show="isEditOpen"
                ...
                <div class="p-6 border-b border-slate-800 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-base font-bold text-white leading-tight">Edit Data Pengguna</h3>
