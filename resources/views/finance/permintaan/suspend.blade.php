...
<div class="space-y-6"
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div class="p-4 rounded-2xl bg-amber-50/70 dark:bg-slate-900/90 border border-amber-200 dark:border-amber-500/20 shadow-md">
            ...
            <div class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Status (1011) Request Suspend</div>
        </div>
        ...
        <div class="p-4 rounded-2xl bg-cyan-50/70 dark:bg-slate-900/90 border border-cyan-200 dark:border-cyan-500/20 shadow-md">
            ...
            <div class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Status (1018) Req Unsuspend</div>
        </div>
        ...
        <div class="p-4 rounded-2xl bg-emerald-50/70 dark:bg-slate-900/90 border border-emerald-200 dark:border-emerald-500/20 shadow-md">
            <div class="flex items-center justify-between">
                <span class="text-[11px] font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider">Selesai Unsuspend</span>
                <span class="w-2 h-2 rounded-full bg-emerald-500 dark:bg-emerald-400"></span>
            </div>
            <div class="mt-2 text-2xl font-black text-slate-900 dark:text-white">{{ number_format($countUnsuspend) }}</div>
            <div class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Status (1013) Normal Kembali</div>
        </div>
    </div>
                    ...
                    @forelse($suspends as $s)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition">
                            ...
                            <td class="py-3.5 px-4">
                                <span class="font-mono font-bold text-rose-600 dark:text-rose-400">{{ $s->kode_suspend }}</span>
                                <div class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Mulai: {{ $s->suspend_start ?? '-' }}</div>
                            </td>

                            <!-- Pelanggan -->
                            <td class="py-3.5 px-4">
                                <div class="font-bold text-slate-900 dark:text-white">{{ $s->nama_pelanggan ?? '-' }}</div>
                                <div class="text-[11px] font-mono text-slate-500 dark:text-slate-400">
                                    No: <a href="{{ route('teknik.pelanggan.profile', $s->nomor_internet) }}" class="text-blue-600 dark:text-blue-400 hover:underline" title="Buka Profile Pelanggan">{{ $s->nomor_internet }}</a>
                                </div>
                            ...
                            <td class="py-3.5 px-4 max-w-xs">
                                <div class="text-slate-700 dark:text-slate-300 font-medium">{{ $s->desc_suspend ?? 'Menunggak jatuh tempo' }}</div>
                                @if($s->user_create)
                            ...
                            <td class="py-3.5 px-4 text-center">
                                <div class="text-[11px] text-slate-700 dark:text-slate-300">{{ $s->suspend_end ?? '-' }}</div>
                            </td>
                            ...
                            <td class="py-3.5 px-4 text-center">
                                ...
                                @elseif($s->status_suspend == '14')
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold bg-slate-100 text-slate-600 border border-slate-200 dark:bg-slate-800 dark:text-slate-400">
                                        <span>Batal</span>
                                    </span>
                                @else
                                    <span class="px-2 py-0.5 rounded text-[11px] bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400">Status {{ $s->status_suspend }}</span>
                                @endif
                            </td>
    ...
    <div x-show="createModalOpen"
        ...
        <div @click.away="createModalOpen = false"
            ...
            <form action="{{ route('finance.permintaan.suspend.store') }}" method="POST" class="space-y-4">
                ...
                <div class="relative">
                    <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Cari Pelanggan Menunggak (No. Internet / Nama) <span class="text-rose-500 dark:text-rose-400">*</span></label>
                    <input type="text"
                           ...
                           class="w-full text-xs px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-rose-500 font-medium">
                    ...
                    <div x-show="customerResults.length > 0"
                        ...
                        <template x-for="cust in customerResults" :key="cust.nomor_internet">
                            <div @click="selectCustomer(cust)"
                                 class="p-2.5 hover:bg-rose-50 dark:hover:bg-rose-600/20 cursor-pointer text-xs transition">
                                ...
                                <div class="flex items-center gap-2 text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                                    <span class="font-mono text-rose-600 dark:text-rose-400" x-text="'No: ' + cust.nomor_internet"></span>
                                    <span>&bull;</span>
                ...
                <template x-if="selectedCustomer">
                    <div class="p-3 rounded-xl bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 text-xs space-y-1">
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Keterangan / Alasan Suspend <span class="text-rose-500 dark:text-rose-400">*</span></label>
                    <textarea name="desc_suspend"
                              rows="3"
                              required
                              placeholder="Contoh: Melewati batas pembayaran tanggal jatuh tempo periode berjalan..."
                              class="w-full text-xs px-3.5 py-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-rose-500">Melewati batas pembayaran yang telah ditentukan</textarea>
                </div>

                <div class="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-3">
                    <button type="button"
                            @click="createModalOpen = false"
                            class="px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-bold transition cursor-pointer">
                        Batal
                    </button>
                    <button type="submit"
