...
<div class="space-y-6"
    ...
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-blue-900/80 via-indigo-900/60 to-slate-900 border border-blue-500/20 p-6 shadow-2xl backdrop-blur-xl">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 relative z-10">
            ...
            <div class="flex flex-wrap items-center gap-2.5">
                ...
                <a href="{{ route('noc.olt') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-200 text-slate-200 text-xs font-bold border border-slate-200 transition">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
        ...
        <!-- 1. Total OLT -->
        <div class="p-5 rounded-2xl bg-white border border-slate-200 backdrop-blur-xl shadow-xl shadow-slate-200/40 flex items-center justify-between">
            <div>
                <span class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Total OLT Aktif</span>
                <div class="text-2xl font-black text-slate-900 mt-1">{{ $totalOlt }} <span class="text-xs font-normal text-slate-400">Node</span></div>
        ...
        <div class="p-5 rounded-2xl bg-white border border-slate-200 backdrop-blur-xl shadow-xl shadow-slate-200/40 flex items-center justify-between">
            <div>
                <span class="text-xs font-semibold text-slate-500 uppercase tracking-wider">ODP Terpasang</span>
                <div class="text-2xl font-black text-slate-900 mt-1">{{ $totalOdp }} <span class="text-xs font-normal text-slate-400">Titik</span></div>
        ...
        <div class="p-5 rounded-2xl bg-white border border-slate-200 backdrop-blur-xl shadow-xl shadow-slate-200/40 flex items-center justify-between">
            <div>
                <span class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Pelanggan Aktif</span>
                <div class="text-2xl font-black text-slate-900 mt-1">{{ number_format($pelangganAktif, 0, ',', '.') }}</div>
        ...
        <div class="p-5 rounded-2xl bg-white border border-slate-200 backdrop-blur-xl shadow-xl shadow-slate-200/40 flex items-center justify-between">
            <div>
                <span class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Antrean Aktivasi</span>
                            ...
                            @forelse($recentAktivasi as $item)
                                <tr class="hover:bg-slate-50 transition">
                                    <td class="py-3.5 px-4">
                                        ...
                                        <div class="font-semibold text-slate-800 uppercase mt-0.5">{{ $item->nama_pelanggan }}</div>
                                    </td>
                                    <td class="py-3.5 px-4">
                                        <div class="font-medium text-slate-700">{{ $item->nama_kategori_bandwith ?: ($item->alias_nama_kategori ?: 'INTERNET') }}</div>
                                        <div class="text-[11px] text-blue-500 font-bold font-mono">{{ $item->nominal_bandwith ?: '10' }} Mbps</div>
                                    </td>
                                    <td class="py-3.5 px-4 text-slate-600 truncate max-w-[200px]">
        ...
        <div class="lg:col-span-5 space-y-4">
            ...
            <div class="bg-white backdrop-blur-xl border border-slate-200 rounded-2xl p-5 shadow-xl shadow-slate-200/40 space-y-4">
                <div class="flex items-center justify-between border-b border-slate-200 pb-3">
                    <span class="text-xs font-bold text-slate-800">Daftar OLT Gateway</span>
                ...
                <div class="space-y-2.5">
                    @forelse($olts as $olt)
                        <div class="p-3 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-between">
                            <div>
                                <div class="text-xs font-bold text-slate-900 font-mono">{{ $olt->name_olt }}</div>
                ...
                <div class="pt-3 border-t border-slate-200">
                    <div class="flex items-center justify-between pb-2">
                        <span class="text-xs font-bold text-slate-800">Point of Presence (POP)</span>
                        <a href="{{ route('noc.pop') }}" class="text-[11px] text-blue-500 hover:underline">Lihat Semua &rarr;</a>
                    </div>
                    <div class="flex flex-wrap gap-1.5 pt-1">
                        @foreach($pops->take(6) as $pop)
                            <span class="px-2.5 py-1 rounded-lg text-[10px] font-semibold bg-slate-100 text-slate-700 border border-slate-200">
                                📍 {{ $pop->nama_pop }}
                            </span>
                        @endforeach
                    </div>
