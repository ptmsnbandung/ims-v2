...
<div class="space-y-6"
    ...
    <div x-show="popModalOpen" x-cloak class="fixed inset-0 z-50 overflow-y-auto" role="dialog">
        <div class="fixed inset-0 bg-slate-100 backdrop-blur-sm" @click="popModalOpen = false"></div>
        <div class="flex min-h-full items-center justify-center p-4">
            <div class="relative w-full max-w-md bg-white border border-slate-200 rounded-2xl shadow-2xl p-6 space-y-4">
                <div class="flex items-center justify-between border-b border-slate-200 pb-3">
                    <h3 class="text-sm font-bold text-slate-900">Tambah Node POP Baru</h3>
                ...
                <form action="{{ route('noc.pop.store') }}" method="POST" class="space-y-4 text-xs">
                    @csrf
                    <div>
                        <label class="block font-semibold text-slate-700 mb-1">Kode POP <span class="text-rose-500">*</span></label>
                        <input type="text" name="kode_pop" required placeholder="Contoh: POP-SRG" class="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-300 text-slate-900">
                    </div>
                    <div>
                        <label class="block font-semibold text-slate-700 mb-1">Nama POP <span class="text-rose-500">*</span></label>
                        <input type="text" name="nama_pop" required placeholder="Contoh: POP SOREANG UTAMA" class="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-300 text-slate-900">
                    </div>
                    <div>
                        <label class="block font-semibold text-slate-700 mb-1">Deskripsi / Alamat Server Node</label>
                        <textarea name="desc_pop" rows="2" placeholder="Detail server / lokasi rak POP..." class="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-300 text-slate-900"></textarea>
                    </div>
                    <div class="pt-3 border-t border-slate-200 flex justify-end gap-2">
                        <button type="button" @click="popModalOpen = false" class="px-4 py-2 rounded-xl bg-slate-200 text-slate-700 font-semibold">Batal</button>
