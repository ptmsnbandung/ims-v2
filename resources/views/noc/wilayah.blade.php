...
<div class="space-y-6"
    ...
    <div x-show="wilayahModalOpen" x-cloak class="fixed inset-0 z-50 overflow-y-auto" role="dialog">
        <div class="fixed inset-0 bg-slate-100 backdrop-blur-sm" @click="wilayahModalOpen = false"></div>
        <div class="flex min-h-full items-center justify-center p-4">
            <div class="relative w-full max-w-md bg-white border border-slate-200 rounded-2xl shadow-2xl p-6 space-y-4">
                <div class="flex items-center justify-between border-b border-slate-200 pb-3">
                    <h3 class="text-sm font-bold text-slate-900">Tambah Wilayah Perangkat Baru</h3>
                ...
                <form action="{{ route('noc.wilayah.store') }}" method="POST" class="space-y-4 text-xs">
                    @csrf
                    <div>
                        <label class="block font-semibold text-slate-700 mb-1">Kode Wilayah <span class="text-rose-500">*</span></label>
                        <input type="text" name="kode_w" placeholder="Contoh: W4" required
                               class="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block font-semibold text-slate-700 mb-1">Nama Wilayah Perangkat <span class="text-rose-500">*</span></label>
                        <input type="text" name="name_w" placeholder="Contoh: KOTA CIMAHI" required
                               class="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block font-semibold text-slate-700 mb-1">Kapasitas Slot</label>
                    ...
                    <div>
                        ...
                        <textarea name="note_w" rows="2" placeholder="Catatan opsional..."
                                  class="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 focus:ring-2 focus:ring-blue-500"></textarea>
                    </div>
                    <div class="pt-3 border-t border-slate-200 flex justify-end gap-2">
                        <button type="button" @click="wilayahModalOpen = false" class="px-4 py-2 rounded-xl bg-slate-200 text-slate-700 font-semibold">Batal</button>
