...
<head>
    ...
    <title>Surat Tugas Survey - {{ $customer->nomor_internet }} - {{ $customer->nama_pelanggan }}</title>
...
@php
    ...
    // Tanggal Survey
    $tglSurvey = $instalasi->survey_date_start 
        ?? ($customer->survey_date_start ?? ($customer->date_create ?? now()->format('Y-m-d')));
    $formattedTglSurvey = \Carbon\Carbon::parse($tglSurvey)->translatedFormat('d F Y');
    $bulanSurat = date('m', strtotime($tglSurvey));
    $tahunSurat = date('Y', strtotime($tglSurvey));
    ...
    // Tim Teknisi Survey
    $teamNames = [];
    if (isset($teamSurvey) && $teamSurvey->count() > 0) {
        $teamNames = $teamSurvey->pluck('nama_karyawan')->toArray();
    } elseif (!empty($instalasi->survey_team)) {
        $teamNames = array_map('trim', explode(',', $instalasi->survey_team));
    }
    $teknisiString = !empty($teamNames) ? implode(', ', $teamNames) : 'Tim Teknisi Lapangan PT MSN';
    ...
    // Detail Pekerjaan / Catatan Survey
    $detailPekerjaan = $instalasi->survey_note 
        ?? ($customer->note_request ?? 'Penarikan kabel FO ke ODP terdekat');

    // Penanggung Jawab (Sesuai Template Master Resmi PT MSN)
    $penanggungJawab = 'IPIN ARIPIN';
@endphp

    <!-- Top Floating Toolbar (Hidden on Print) -->
    <div class="screen-toolbar no-print">
        <div class="toolbar-info">
            <span class="toolbar-badge">SURAT TUGAS SURVEY</span>
            <div>
        ...
        <div class="paper-sheet" id="paper-sheet-container">
            ...
            <div class="survey-body">
                ...
                <div class="survey-title-block">
                    <div class="survey-title">SURAT TUGAS SURVEY</div>
                    <div class="survey-nomor">Nomor : &nbsp;&nbsp;<strong>{{ $nomorSurat }}</strong></div>
                </div>

                <!-- Paragraph 1: Penugasan -->
                <p class="survey-intro">
                    Dengan surat ini pada tanggal, <strong>{{ $formattedTglSurvey }}</strong> PT Media Solusi Network menugaskan tim teknisi, <strong>{{ $teknisiString }}</strong>. Untuk melakukan survei lokasi pemasangan jaringan internet ke pada pelanggan baru dengan rincian sebagai berikut :
                </p>

                <!-- Details Table -->
                <table class="survey-details-table">
                    <tr>
                    ...
                    <tr>
                        ...
                        <td class="col-colon">:</td>
                        <td class="col-val">{{ $customer->pic ?: '-' }}</td>
                    </tr>
                    <tr>
            ...
            <!-- 3. FOOTER ASLI (EKSTRAKSI DARI TEMPLATE MASTER PT MSN) -->
    ...
    <script>
        ...
        function downloadPDF() {
            ...
            const filename = `Surat_Tugas_Survey_${nomorInternet}_${customerName}.pdf`;
        ...
        function downloadWord() {
            ...
            try {
                ...
                const wordHtml = `
                    ...
                    <head>
                        <meta charset='utf-8'>
                        <title>Surat Tugas Survey - ${nomorInternet}</title>
                        <!--[if gte mso 9]>
                        <xml>
                            <w:WordDocument>
                                <w:View>Print</w:View>
                        ...
                        <style>
                            ...
                            .survey-title { text-align: center; font-size: 16pt; font-weight: bold; margin-top: 10px; }
                            .survey-nomor { text-align: center; font-size: 10pt; margin-bottom: 20px; }
                            .survey-intro { text-align: justify; margin-bottom: 15px; }
                            .survey-details-table td { padding: 4px 0; vertical-align: top; }
                            .col-label { width: 150px; font-weight: normal; }
                            .col-colon { width: 20px; text-align: center; }
                            .col-val { font-weight: bold; text-transform: uppercase; }
                            .signature-table td { text-align: center; width: 50%; padding: 10px; }
                            .sig-role { margin-bottom: 75px; font-weight: normal; }
                            .sig-name { font-weight: normal; text-transform: uppercase; }
                ...
                link.download = `Surat_Tugas_Survey_${nomorInternet}_${customerName}.doc`;
        ...
        function downloadPNG() {
            ...
            const element = document.getElementById('paper-sheet-container');
            const filename = `Surat_Tugas_Survey_${nomorInternet}_${customerName}.png`;
