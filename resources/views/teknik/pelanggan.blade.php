        ...
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-4">
            <div class="flex items-center gap-1.5 p-1 bg-slate-100 border border-slate-200 rounded-xl">
                ...
                <button type="button" 
                    ...
                    <span class="px-1.5 py-0.5 rounded text-[10px]" :class="activeTab === 'aktif' ? 'bg-white/20 text-white font-extrabold' : 'bg-slate-200 text-slate-700'">
                ...
                <button type="button" 
                    ...
                    <span>Terminasi</span>
                    <span class="px-1.5 py-0.5 rounded text-[10px]" :class="activeTab === 'terminasi' ? 'bg-white/20 text-white font-extrabold' : 'bg-slate-200 text-slate-700'">
                ...
                <button type="button" 
                    ...
                    <span>Suspend</span>
                    <span class="px-1.5 py-0.5 rounded text-[10px]" :class="activeTab === 'suspend' ? 'bg-white/20 text-white font-extrabold' : 'bg-slate-200 text-slate-700'">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                @foreach($layananList as $layanan)
                    ...
                    <a href="{{ route('teknik.pelanggan', ['status' => '20', 'layanan' => $layanan->kode_kategori_bandwith]) }}" 
                       class="group p-3.5 rounded-xl border transition-all duration-150 shadow-sm {{ $isActiveFilter ? 'bg-blue-50/80 border-blue-500 ring-2 ring-blue-500/30' : 'bg-white hover:bg-slate-50 border-slate-200 hover:border-slate-300' }} flex flex-col justify-between min-h-[82px]">
                ...
                <a href="{{ route('teknik.pelanggan', ['status' => '20']) }}" 
                   class="group p-3.5 rounded-xl border transition-all duration-150 shadow-sm {{ (($filters['status'] ?? '') == '20' && empty($filters['layanan'])) ? 'bg-blue-50/80 border-blue-500 ring-2 ring-blue-500/30' : 'bg-white hover:bg-slate-50 border-slate-200 hover:border-slate-300' }} flex flex-col justify-between min-h-[82px]">
                    <div class="flex items-center justify-between">
                    ...
                    <div class="mt-2.5 flex items-baseline justify-between">
                        ...
                        <span class="text-[11px] text-blue-700/80 font-semibold">User</span>
                    </div>
                </a>
            </div>
        </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                @foreach($layananList as $layanan)
                    ...
                    <a href="{{ route('teknik.pelanggan', ['status' => '23', 'layanan' => $layanan->kode_kategori_bandwith]) }}" 
                       class="group p-3.5 rounded-xl border transition-all duration-150 shadow-sm {{ $isActiveFilter ? 'bg-rose-50/80 border-rose-500 ring-2 ring-rose-500/30' : 'bg-white hover:bg-slate-50 border-slate-200 hover:border-slate-300' }} flex flex-col justify-between min-h-[82px]">
                        <div class="flex items-center justify-between">
                ...
                <a href="{{ route('teknik.pelanggan', ['status' => '23']) }}" 
                   class="group p-3.5 rounded-xl border transition-all duration-150 shadow-sm {{ (($filters['status'] ?? '') == '23' && empty($filters['layanan'])) ? 'bg-rose-50/80 border-rose-500 ring-2 ring-rose-500/30' : 'bg-white hover:bg-slate-50 border-slate-200 hover:border-slate-300' }} flex flex-col justify-between min-h-[82px]">
                    <div class="flex items-center justify-between">
                    ...
                    <div class="mt-2.5 flex items-baseline justify-between">
                        ...
                        <span class="text-[11px] text-rose-700/80 font-semibold">User</span>
                    </div>
                </a>
            </div>
        </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                @foreach($layananList as $layanan)
                    ...
                    <a href="{{ route('teknik.pelanggan', ['status' => '21', 'layanan' => $layanan->kode_kategori_bandwith]) }}" 
                       class="group p-3.5 rounded-xl border transition-all duration-150 shadow-sm {{ $isActiveFilter ? 'bg-amber-50/80 border-amber-500 ring-2 ring-amber-500/30' : 'bg-white hover:bg-slate-50 border-slate-200 hover:border-slate-300' }} flex flex-col justify-between min-h-[82px]">
                        <div class="flex items-center justify-between">
                ...
                <a href="{{ route('teknik.pelanggan', ['status' => '21']) }}" 
                   class="group p-3.5 rounded-xl border transition-all duration-150 shadow-sm {{ (($filters['status'] ?? '') == '21' && empty($filters['layanan'])) ? 'bg-amber-50/80 border-amber-500 ring-2 ring-amber-500/30' : 'bg-white hover:bg-slate-50 border-slate-200 hover:border-slate-300' }} flex flex-col justify-between min-h-[82px]">
                    <div class="flex items-center justify-between">
                        <span class="px-2.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-amber-500 text-slate-900 shadow-sm">
                            Total Suspend
                        </span>
                    ...
                    <div class="mt-2.5 flex items-baseline justify-between">
                        ...
                        <span class="text-[11px] text-amber-700/80 font-semibold">User</span>
                    </div>
                </a>
            </div>
        </div>
    ...
    <div class="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-sm">
        <form method="GET" action="{{ route('teknik.pelanggan') }}" id="filterForm">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3.5 items-end">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('teknik.pelanggan') }}" 
                       class="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 text-xs font-semibold transition duration-150 shadow-sm">
                        <svg class="w-3.5 h-3.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
            ...
            <div class="mt-4 flex items-center justify-between border-t border-slate-200 pt-3">
                ...
                <div>
                    <button type="submit" class="px-4 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-xs text-white font-semibold transition shadow-sm">
                        Terapkan Filter
                    </button>
                </div>
            </div>
    ...
    <div class="bg-white border border-slate-200/90 rounded-2xl shadow-sm overflow-hidden">
                    ...
                    @forelse($pelanggan as $item)
                        <tr class="hover:bg-slate-50/80 transition duration-150">
                            ...
                            <td class="py-4 px-4 align-top max-w-xs">
                                <span class="font-bold text-slate-800 uppercase text-[11px] tracking-wide block mb-1">
                            ...
                            <td class="py-4 px-4 align-top space-y-2">
                                ...
                                @if($item->date_update)
                                    <div class="text-[10px] text-slate-400">
                                        Updated: <span class="text-slate-600">{{ \Carbon\Carbon::parse($item->date_update)->translatedFormat('d M Y H:i') }} WIB</span>
                                    </div>
                                @endif
                            </td>
                            ...
                            <td class="py-4 px-4 align-top text-xs space-y-1">
                                <div class="text-slate-800 font-semibold text-[11px]">
                            ...
                            <td class="py-4 px-4 align-top text-center text-xs">
                                <div class="flex items-center justify-center gap-1.5">
                                    <a href="{{ route('teknik.dokumen.langganan', $item->nomor_internet) }}?download=pdf" 
                                       target="_blank"
                                       class="p-1.5 rounded-lg bg-slate-100 hover:bg-emerald-50 text-emerald-600 hover:text-emerald-700 border border-slate-200 hover:border-emerald-300 transition" 
                                    ...
                                    <a href="{{ route('teknik.dokumen.langganan', $item->nomor_internet) }}" 
                                       target="_blank"
                                       class="p-1.5 rounded-lg bg-slate-100 hover:bg-blue-50 text-blue-600 hover:text-blue-700 border border-slate-200 hover:border-blue-300 transition" 
                                    ...
                                    <a href="{{ route('teknik.pelanggan.profile', $item->nomor_internet) }}" 
                                       class="p-1.5 rounded-lg bg-slate-100 hover:bg-indigo-50 text-indigo-600 hover:text-indigo-700 border border-slate-200 hover:border-indigo-300 transition" 
        ...
        <div class="px-5 py-4 border-t border-slate-200 bg-slate-50 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-1">
                <a href="{{ $pelanggan->url(1) }}" class="px-2.5 py-1.5 rounded-lg border text-xs font-medium transition {{ $pelanggan->onFirstPage() ? 'border-slate-200 text-slate-300 pointer-events-none' : 'border-slate-300 bg-white text-slate-700 hover:bg-slate-100 hover:text-slate-900' }}">First</a>
                <a href="{{ $pelanggan->previousPageUrl() }}" class="px-2.5 py-1.5 rounded-lg border text-xs font-medium transition {{ $pelanggan->onFirstPage() ? 'border-slate-200 text-slate-300 pointer-events-none' : 'border-slate-300 bg-white text-slate-700 hover:bg-slate-100 hover:text-slate-900' }}">Previous</a>
                <span class="px-3 py-1.5 rounded-lg bg-blue-600 text-white text-xs font-bold shadow-sm">{{ $pelanggan->currentPage() }}</span>
                <a href="{{ $pelanggan->nextPageUrl() }}" class="px-2.5 py-1.5 rounded-lg border text-xs font-medium transition {{ $pelanggan->hasMorePages() ? 'border-slate-300 bg-white text-slate-700 hover:bg-slate-100 hover:text-slate-900' : 'border-slate-200 text-slate-300 pointer-events-none' }}">Next</a>
                <a href="{{ $pelanggan->url($pelanggan->lastPage()) }}" class="px-2.5 py-1.5 rounded-lg border text-xs font-medium transition {{ $pelanggan->hasMorePages() ? 'border-slate-300 bg-white text-slate-700 hover:bg-slate-100 hover:text-slate-900' : 'border-slate-200 text-slate-300 pointer-events-none' }}">Last</a>
            </div>
        </div>
    </div>
