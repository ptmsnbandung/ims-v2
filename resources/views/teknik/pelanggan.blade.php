...
<div class="space-y-6"
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white border border-slate-200/90 rounded-2xl p-4 sm:p-5 shadow-sm">
        <!-- Breadcrumb & Title -->
        <div>
        ...
        <div class="flex flex-wrap items-center gap-2.5">
            <div class="px-3.5 py-1.5 rounded-xl bg-slate-50 border border-slate-200 flex items-center gap-2 text-xs shadow-xs">
            ...
            <div class="px-3.5 py-1.5 rounded-xl bg-emerald-50 border border-emerald-200 flex items-center gap-2 text-xs shadow-xs">
            ...
            <div class="px-3.5 py-1.5 rounded-xl bg-rose-50 border border-rose-200 flex items-center gap-2 text-xs shadow-xs">
            ...
            <div class="px-3.5 py-1.5 rounded-xl bg-amber-50 border border-amber-200 flex items-center gap-2 text-xs shadow-xs">
    ...
    <div class="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-sm space-y-5">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
            <div class="flex items-center gap-1.5 p-1 bg-slate-100/90 border border-slate-200/90 rounded-xl">
                <!-- Tab Aktif -->
                <button type="button" 
                        @click="activeTab = 'aktif'"
                        :class="activeTab === 'aktif' ? 'bg-cyan-600 text-white shadow-sm font-bold' : 'text-slate-600 hover:text-slate-900 font-medium'"
                        class="flex items-center gap-2 px-4 py-2 rounded-lg text-xs transition duration-150 cursor-pointer">
                    <span class="w-2 h-2 rounded-full" :class="activeTab === 'aktif' ? 'bg-white' : 'bg-cyan-600'"></span>
                    <span>Pelanggan Aktif</span>
                    <span class="px-1.5 py-0.5 rounded text-[10px]" :class="activeTab === 'aktif' ? 'bg-white/20 text-white font-extrabold' : 'bg-white text-cyan-700 font-bold border border-slate-200'">
                ...
                <button type="button" 
                    ...
                    <span>Terminasi</span>
                    <span class="px-1.5 py-0.5 rounded text-[10px]" :class="activeTab === 'terminasi' ? 'bg-white/20 text-white font-extrabold' : 'bg-white text-rose-700 font-bold border border-slate-200'">
                ...
                <button type="button" 
                        ...
                        :class="activeTab === 'suspend' ? 'bg-amber-600 text-white shadow-sm font-bold' : 'text-slate-600 hover:text-slate-900 font-medium'"
                        class="flex items-center gap-2 px-4 py-2 rounded-lg text-xs transition duration-150 cursor-pointer">
                    <span class="w-2 h-2 rounded-full" :class="activeTab === 'suspend' ? 'bg-white' : 'bg-amber-500'"></span>
                    <span>Suspend</span>
                    <span class="px-1.5 py-0.5 rounded text-[10px]" :class="activeTab === 'suspend' ? 'bg-white/20 text-white font-extrabold' : 'bg-white text-amber-700 font-bold border border-slate-200'">
                ...
                <button type="button" 
                        @click="activeTab = 'all'"
                        :class="activeTab === 'all' ? 'bg-blue-600 text-white shadow-sm font-bold' : 'text-slate-600 hover:text-slate-900 font-medium'"
                        class="flex items-center gap-2 px-4 py-2 rounded-lg text-xs transition duration-150 cursor-pointer">
            ...
            <!-- Hint text -->
            <span class="text-[11px] text-slate-500 hidden lg:inline-block font-medium">
        ...
        <div x-show="activeTab === 'aktif' || activeTab === 'all'" x-cloak class="space-y-3">
            <div class="flex items-center justify-between" x-show="activeTab === 'all'">
                <h4 class="text-xs font-bold text-cyan-700 uppercase tracking-wider flex items-center gap-2">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                @foreach($layananList as $layanan)
                    ...
                    <a href="{{ route('teknik.pelanggan', ['status' => '20', 'layanan' => $layanan->kode_kategori_bandwith]) }}" 
                       class="group p-3.5 rounded-xl border transition-all duration-150 {{ $isActiveFilter ? 'bg-cyan-50 border-cyan-500 ring-2 ring-cyan-500/30 shadow-sm' : 'bg-slate-50/70 hover:bg-white border-slate-200/90 hover:border-slate-300 hover:shadow-sm' }} flex flex-col justify-between min-h-[82px]">
                        <div class="flex items-center justify-between">
                            <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-cyan-100 text-cyan-800 border border-cyan-200 group-hover:border-cyan-300 transition truncate max-w-[120px]">
                                {{ $layanan->alias_nama_kategori ?: $layanan->nama_kategori_bandwith }}
                            </span>
                            <span class="w-1.5 h-1.5 rounded-full {{ $count > 0 ? 'bg-cyan-600' : 'bg-slate-300' }}"></span>
                        </div>
                        <div class="mt-2.5 flex items-baseline justify-between">
                            <span class="text-base font-bold {{ $count > 0 ? 'text-slate-900 group-hover:text-cyan-700' : 'text-slate-400' }}">
                                {{ number_format($count) }}
                            </span>
                ...
                <a href="{{ route('teknik.pelanggan', ['status' => '20']) }}" 
                   class="group p-3.5 rounded-xl border transition-all duration-150 {{ (($filters['status'] ?? '') == '20' && empty($filters['layanan'])) ? 'bg-cyan-50 border-cyan-500 ring-2 ring-cyan-500/30' : 'bg-slate-50/70 hover:bg-white border-slate-200/90 hover:border-slate-300 hover:shadow-sm' }} flex flex-col justify-between min-h-[82px]">
                    <div class="flex items-center justify-between">
                        <span class="px-2.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-cyan-600 text-white shadow-xs">
                            Total Aktif
                        </span>
                        <svg class="w-3.5 h-3.5 text-cyan-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
                        </svg>
                    </div>
                    <div class="mt-2.5 flex items-baseline justify-between">
                        <span class="text-base font-extrabold text-cyan-700 group-hover:text-cyan-800">
                            {{ number_format($bwCounts['total_aktif']) }}
                        </span>
                        <span class="text-[11px] text-cyan-600 font-semibold">User</span>
                    </div>
                </a>
            </div>
        </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                @foreach($layananList as $layanan)
                    ...
                    <a href="{{ route('teknik.pelanggan', ['status' => '23', 'layanan' => $layanan->kode_kategori_bandwith]) }}" 
                       class="group p-3.5 rounded-xl border transition-all duration-150 {{ $isActiveFilter ? 'bg-rose-50 border-rose-500 ring-2 ring-rose-500/30 shadow-sm' : 'bg-slate-50/70 hover:bg-white border-slate-200/90 hover:border-slate-300 hover:shadow-sm' }} flex flex-col justify-between min-h-[82px]">
                        <div class="flex items-center justify-between">
                            <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-rose-100 text-rose-800 border border-rose-200 group-hover:border-rose-300 transition truncate max-w-[120px]">
                                {{ $layanan->alias_nama_kategori ?: $layanan->nama_kategori_bandwith }}
                            </span>
                            <span class="w-1.5 h-1.5 rounded-full {{ $count > 0 ? 'bg-rose-600' : 'bg-slate-300' }}"></span>
                        </div>
                        <div class="mt-2.5 flex items-baseline justify-between">
                            <span class="text-base font-bold {{ $count > 0 ? 'text-slate-900 group-hover:text-rose-700' : 'text-slate-400' }}">
                                {{ number_format($count) }}
                            </span>
                            <span class="text-[11px] text-slate-500 font-medium">User</span>
                        </div>
                ...
                <a href="{{ route('teknik.pelanggan', ['status' => '23']) }}" 
                   class="group p-3.5 rounded-xl border transition-all duration-150 {{ (($filters['status'] ?? '') == '23' && empty($filters['layanan'])) ? 'bg-rose-50 border-rose-500 ring-2 ring-rose-500/30' : 'bg-slate-50/70 hover:bg-white border-slate-200/90 hover:border-slate-300 hover:shadow-sm' }} flex flex-col justify-between min-h-[82px]">
                    <div class="flex items-center justify-between">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                @foreach($layananList as $layanan)
                    ...
                    <a href="{{ route('teknik.pelanggan', ['status' => '21', 'layanan' => $layanan->kode_kategori_bandwith]) }}" 
                       class="group p-3.5 rounded-xl border transition-all duration-150 {{ $isActiveFilter ? 'bg-amber-50 border-amber-500 ring-2 ring-amber-500/30 shadow-sm' : 'bg-slate-50/70 hover:bg-white border-slate-200/90 hover:border-slate-300 hover:shadow-sm' }} flex flex-col justify-between min-h-[82px]">
                        <div class="flex items-center justify-between">
                            <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-amber-100 text-amber-800 border border-amber-200 group-hover:border-amber-300 transition truncate max-w-[120px]">
                                {{ $layanan->alias_nama_kategori ?: $layanan->nama_kategori_bandwith }}
                            </span>
                            <span class="w-1.5 h-1.5 rounded-full {{ $count > 0 ? 'bg-amber-600' : 'bg-slate-300' }}"></span>
                        </div>
                        <div class="mt-2.5 flex items-baseline justify-between">
                            <span class="text-base font-bold {{ $count > 0 ? 'text-slate-900 group-hover:text-amber-700' : 'text-slate-400' }}">
                                {{ number_format($count) }}
                            </span>
                            <span class="text-[11px] text-slate-500 font-medium">User</span>
                        </div>
                ...
                <a href="{{ route('teknik.pelanggan', ['status' => '21']) }}" 
                   class="group p-3.5 rounded-xl border transition-all duration-150 {{ (($filters['status'] ?? '') == '21' && empty($filters['layanan'])) ? 'bg-amber-50 border-amber-500 ring-2 ring-amber-500/30' : 'bg-slate-50/70 hover:bg-white border-slate-200/90 hover:border-slate-300 hover:shadow-sm' }} flex flex-col justify-between min-h-[82px]">
                    <div class="flex items-center justify-between">
                        <span class="px-2.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-amber-600 text-white shadow-xs">
                            Total Suspend
                        </span>
    ...
    <div class="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-sm">
        <form method="GET" action="{{ route('teknik.pelanggan') }}" id="filterForm">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3.5 items-end">
                ...
                <div>
                    <label class="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1.5">Cari Pelanggan</label>
                    <input type="text" 
                           name="search" 
    ...
    <div class="bg-white border border-slate-200/90 rounded-2xl shadow-sm overflow-hidden">
                            ...
                            <td class="py-4 px-4 align-top space-y-2">
                                ...
                                @if($item->date_update)
                                    <div class="text-[10px] text-slate-500">
                                        Updated: <span class="text-slate-700 font-medium">{{ \Carbon\Carbon::parse($item->date_update)->translatedFormat('d M Y H:i') }} WIB</span>
                                    </div>
                                @endif
                            </td>
                            ...
                            <td class="py-4 px-4 align-top text-center text-xs">
                                <div class="flex items-center justify-center gap-1.5">
                                    ...
                                    <a href="{{ route('teknik.dokumen.langganan', $item->nomor_internet) }}" 
                                       target="_blank"
                                       class="p-1.5 rounded-lg bg-sky-50 hover:bg-sky-100 text-sky-700 border border-sky-200 transition" 
                                    ...
                                    <a href="{{ route('teknik.pelanggan.profile', $item->nomor_internet) }}" 
                                       class="p-1.5 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 transition" 
        ...
        <div class="px-5 py-4 border-t border-slate-200 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="text-xs text-slate-500">
                Showing <span class="text-slate-800 font-semibold">{{ $pelanggan->firstItem() ?? 0 }}</span> 
                to <span class="text-slate-800 font-semibold">{{ $pelanggan->lastItem() ?? 0 }}</span> 
                of <span class="text-slate-800 font-semibold">{{ $pelanggan->total() }}</span> entries
            </div>

            <div class="flex items-center gap-1">
                <a href="{{ $pelanggan->url(1) }}" class="px-2.5 py-1.5 rounded-lg border text-xs font-medium transition {{ $pelanggan->onFirstPage() ? 'border-slate-200 text-slate-400 pointer-events-none' : 'border-slate-300 bg-white text-slate-700 hover:bg-slate-50' }}">First</a>
                <a href="{{ $pelanggan->previousPageUrl() }}" class="px-2.5 py-1.5 rounded-lg border text-xs font-medium transition {{ $pelanggan->onFirstPage() ? 'border-slate-200 text-slate-400 pointer-events-none' : 'border-slate-300 bg-white text-slate-700 hover:bg-slate-50' }}">Previous</a>
                <span class="px-3 py-1.5 rounded-lg bg-blue-600 text-white text-xs font-bold shadow-xs">{{ $pelanggan->currentPage() }}</span>
                <a href="{{ $pelanggan->nextPageUrl() }}" class="px-2.5 py-1.5 rounded-lg border text-xs font-medium transition {{ $pelanggan->hasMorePages() ? 'border-slate-300 bg-white text-slate-700 hover:bg-slate-50' : 'border-slate-200 text-slate-400 pointer-events-none' }}">Next</a>
                <a href="{{ $pelanggan->url($pelanggan->lastPage()) }}" class="px-2.5 py-1.5 rounded-lg border text-xs font-medium transition {{ $pelanggan->hasMorePages() ? 'border-slate-300 bg-white text-slate-700 hover:bg-slate-50' : 'border-slate-200 text-slate-400 pointer-events-none' }}">Last</a>
            </div>
        </div>
    </div>
