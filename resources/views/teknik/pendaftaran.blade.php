        ...
        <div class="flex min-h-full items-center justify-center p-2 sm:p-4 text-center">
            <div class="relative transform overflow-hidden rounded-2xl bg-white border border-slate-200 text-slate-800 text-left shadow-2xl transition-all sm:my-8 w-full max-w-4xl">
                ...
                <div x-show="isLoadingEdit" class="absolute inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex flex-col items-center justify-center gap-3">
                    ...
                    <span class="text-xs text-slate-300 font-semibold">Memuat Data Pendaftaran...</span>
                </div>
                ...
                <div class="px-6 py-5 border-b border-slate-100 bg-slate-50/50">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-2.5">
                            ...
                            <div>
                                <h3 class="text-base font-bold text-white tracking-wide" id="modal-title" x-text="isEditMode ? 'Edit Pendaftaran: ' + editNomorInternet : 'Form Registration'"></h3>
                                <p class="text-xs text-slate-400" x-text="isEditMode ? 'Perbarui data pendaftaran dan instalasi pelanggan' : 'Pendaftaran pelanggan baru Internet System Management'"></p>
                            </div>
                        </div>
                ...
                <div x-show="stepError" x-cloak class="mx-6 mt-4 p-3 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-400 text-xs flex items-center gap-2">
                                ...
                                <div class="relative border-2 border-dashed border-slate-300 hover:border-blue-500 rounded-xl p-3 text-center bg-slate-50 hover:bg-blue-50/30 transition cursor-pointer flex flex-col items-center justify-center min-h-[110px]">
                                    ...
                                    <template x-if="!fotoKtpPreview">
                                        <div class="space-y-1 flex flex-col items-center">
                                            ...
                                            <span class="text-[11px] text-slate-400 font-medium">Upload Foto KTP</span>
                                        </div>
                                    </template>
                                    <template x-if="fotoKtpPreview">
                                ...
                                <div class="relative border-2 border-dashed border-slate-300 hover:border-blue-500 rounded-xl p-3 text-center bg-slate-50 hover:bg-blue-50/30 transition cursor-pointer flex flex-col items-center justify-center min-h-[110px]">
                                    ...
                                    <template x-if="!fotoRumahPreview">
                                        <div class="space-y-1 flex flex-col items-center">
                                            ...
                                            <span class="text-[11px] text-slate-400 font-medium">Upload Foto Rumah</span>
                                        </div>
                                    </template>
                                    <template x-if="fotoRumahPreview">
                    ...
                    <div x-show="currentStep === 2" class="space-y-4">
                        ...
                        <div class="p-3 rounded-xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-between">
                            <div class="text-xs text-slate-300">
                                Nomor Pelanggan: <strong class="text-blue-400 font-mono text-sm tracking-wide" x-text="editNomorInternet"></strong>
                            </div>
                            <span class="text-[10px] text-slate-400" x-text="isEditMode ? 'Nomor Pelanggan Tetap' : 'Dibuat otomatis oleh sistem'"></span>
                        </div>
                    ...
                    <div class="mt-8 pt-5 border-t border-slate-800 flex items-center justify-between">
                        ...
                        <template x-if="currentStep === 1">
                            <div class="w-full flex items-center justify-between">
                                ...
                                <button type="button" 
                                        @click="nextToStep2()"
                                        class="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold transition shadow-lg shadow-blue-500/25 cursor-pointer">
                        ...
                        <template x-if="currentStep === 2">
                            <div class="w-full flex items-center justify-between">
                                ...
                                <button type="submit" 
                                        class="inline-flex items-center gap-2 px-7 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold transition shadow-lg shadow-emerald-500/25 cursor-pointer">
                                    <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M17.593 3.322c1.1.128 1.907 1.077 1.907 2.185V21L12 17.25 4.5 21V5.507c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0 1 11.186 0Z" />
                                    </svg>
            ...
            <div class="relative transform overflow-hidden rounded-2xl bg-white border border-slate-200 text-slate-800 text-left shadow-2xl transition-all sm:my-8 w-full max-w-4xl">
                ...
                <form method="POST" action="{{ route('teknik.pendaftaran.schedule-survey') }}" enctype="multipart/form-data">
                    ...
                    <div class="px-6 py-4 bg-slate-50 dark:bg-slate-950/80 border-t border-slate-200 dark:border-slate-800 flex items-center justify-end gap-3">
                        ...
                        <button type="submit" 
                                class="inline-flex items-center gap-1.5 px-6 py-2 rounded-xl bg-[#0073b7] hover:bg-[#005a91] text-white text-xs font-bold transition shadow-md shadow-blue-500/20 cursor-pointer">
                            <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M17.593 3.322c1.1.128 1.907 1.077 1.907 2.185V21L12 17.25 4.5 21V5.507c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0 1 11.186 0Z" />
                            </svg>
                            <span>Update</span>
            ...
            <div class="relative transform overflow-hidden rounded-2xl bg-white border border-slate-200 text-slate-800 text-left shadow-2xl transition-all sm:my-8 w-full max-w-5xl">
                ...
                <form method="POST" action="{{ route('teknik.pendaftaran.report-survey') }}" enctype="multipart/form-data">
                    ...
                    <div class="p-6 space-y-6 max-h-[78vh] overflow-y-auto">
                        ...
                        <div class="p-3 rounded-xl bg-rose-500/10 border border-rose-500/20 flex items-center gap-3">
                            <span class="text-sm font-bold text-rose-500">Jadwal Ulang Survey ?</span>
                                ...
                                <div class="space-y-3 pt-2 border-t border-slate-200 dark:border-slate-800">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <button type="button" 
                                                ...
                                                class="px-4 py-1.5 rounded-xl bg-[#00c0ef] hover:bg-[#00a7d0] text-white text-xs font-bold transition shadow-sm cursor-pointer">
                                            Add
                                        </button>
                                    </div>
            ...
            <div class="relative transform overflow-hidden rounded-2xl bg-white border border-slate-200 text-slate-800 text-left shadow-2xl transition-all sm:my-8 w-full max-w-lg">
                ...
                <form method="POST" action="{{ route('teknik.pendaftaran.request-aktivasi') }}">
                    ...
                    <div class="px-6 py-4 bg-slate-50 dark:bg-slate-950/80 border-t border-slate-200 dark:border-slate-800 flex items-center justify-end gap-3">
                        ...
                        <button type="submit" 
                                class="inline-flex items-center gap-2 px-5 py-2 rounded-xl bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-400 hover:to-orange-500 text-white text-xs font-bold transition shadow-md shadow-amber-500/25 cursor-pointer">
                            <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
